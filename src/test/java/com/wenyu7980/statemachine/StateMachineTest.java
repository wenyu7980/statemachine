...
public class StateMachineTest {
    ...
    public void testListener() {
        ...
        stateMachine.sendEvent(data, State.S1, Event.E1, context);
        // 确认调用
        verify(preListener).listener(data, State.S1, Event.E1, context);
        verify(preEvent).listener(data, State.S1, context);
        verify(exitState).listener(data, Event.E1);
        verify(transformListener).listener(data, Event.E1);
        verify(enterState).listener(data, Event.E1);
        verify(postEvent).listener(data, State.S1, context);
        verify(postListener).listener(data, State.S1, Event.E1, context);
    }
}
